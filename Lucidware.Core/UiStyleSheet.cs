...
public class UiStyleSheet
{
    ...
    public static UiStyleSheet LoadFromPath(string path, IoCore assetIo)
    {
        var styleSheet = assetIo.LoadString(path);
        try
        {
    ...
    public static UiStyleSheet Load(string styleSheet, IoCore assetIo)
    {
        UiStyleSheet uiStyleSheet = new UiStyleSheet();
        uiStyleSheet.Source = styleSheet;
        uiStyleSheet.IoCore = assetIo;
        uiStyleSheet.Parse();
        return uiStyleSheet;
    }
            ...
            else
            {
                foreach (var property in styleBox.GetType().GetProperties())
                {
                    if (property.Name == key)
                    {
                        ...
                        {
                            property.SetValue(styleBox, LoadImageTexture(IoCore, value.ToString()));
                        }
    ...
    public Rect2 ToRect2(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
