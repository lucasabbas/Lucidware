...
public partial class UiDocument : Control
{
    ...
    public UiDocument(IoCore ioCore)
    {
        _ioCore = ioCore;
    ...
    public static UiDocument LoadFromPath(IoCore ioCore, string path)
    {
        var doc = new UiDocument(ioCore);
        doc.Load(path);
        return doc;
    }
    ...
    {
        string xml = _ioCore.LoadString(path);
        try
        {
            LoadFromString(xml);
        }
        ...
        {
            throw new Exception("SUML Parse Error in " + path + " : " + e.Message);
        }
    }
    ...
    public void Instantiate(XmlDocument document)
    {
        ...
        XmlNode firstChild = document.GetElementsByTagName("SUML")[0];
        if (firstChild.Name == "SUML")
        {
            ...
            else
            {
                ...
                if (anchorTop != null)
                {
                    AnchorTop = anchorTop.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute anchorLeft = firstChild.Attributes["AnchorLeft"];
                if (anchorLeft != null)
                {
                    AnchorLeft = anchorLeft.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute anchorRight = firstChild.Attributes["AnchorRight"];
                if (anchorRight != null)
                {
                    AnchorRight = anchorRight.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute anchorBottom = firstChild.Attributes["AnchorBottom"];
                if (anchorBottom != null)
                {
                    AnchorBottom = anchorBottom.Value.ParseMacrosToFloat(_ioCore);
                }

                XmlAttribute offsetTop = firstChild.Attributes["OffsetTop"];
                if (offsetTop != null)
                {
                    OffsetTop = offsetTop.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute offsetLeft = firstChild.Attributes["OffsetLeft"];
                if (offsetLeft != null)
                {
                    OffsetLeft = offsetLeft.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute offsetRight = firstChild.Attributes["OffsetRight"];
                if (offsetRight != null)
                {
                    OffsetRight = offsetRight.Value.ParseMacrosToFloat(_ioCore);
                }
                XmlAttribute offsetBottom = firstChild.Attributes["OffsetBottom"];
                if (offsetBottom != null)
                {
                    OffsetBottom = offsetBottom.Value.ParseMacrosToFloat(_ioCore);
                }
            }
                    ...
                    foreach (XmlNode headChildNode in node.ChildNodes)
                    {
                        if (headChildNode.Name == "Document")
                        {
                            if (headChildNode.Attributes["Path"] != null && headChildNode.Attributes["Type"] != null)
                            {
                                ...
                                if (type == "StyleSheet")
                                {
                                    var styleSheet = UiStyleSheet.LoadFromPath(documentPath, _ioCore);
                        ...
                        else if (headChildNode.Name == "Style")
                        {
                            ...
                            var styleSheet = UiStyleSheet.Load(style, _ioCore);
                            StyleSheets.Add(styleSheet);
                        }
    ...
    private void ConstructNodes(string version, XmlNodeList nodes)
    {
        if (version == "1.0")
        {
            ...
            {
                Node gdNode = Construct(node, _ioCore);
                if (gdNode != null)
                {
                    AddChild(gdNode);
                }
    ...
    public Node Construct(XmlNode node, IoCore assetIo)
    {
        if (node.Name == "Script")
        {
        ...
        {
            var template = new UiDocument(_ioCore);
    ...
    public void SetStyles(XmlNode node, Control control)
    {
        foreach (var attribute in node.Attributes)
        {
            ...
            {
                control.AddThemeConstantOverride(styleName, attr.Value.ParseMacrosToInt(_ioCore));
            }
            else if (control.HasThemeFontSizeOverride(styleName))
            {
                control.AddThemeFontSizeOverride(styleName, attr.Value.ParseMacrosToInt(_ioCore));
            }
            ...
            {
                control.AddThemeIconOverride(styleName, LoadImageTexture(_ioCore, attr.Value.ParseMacros()));
            }
        }
    }
    ...
    public void SetObjectValue(object attribute, object obj, PropertyInfo[] properties)
    {
        ...
        foreach (var property in properties)
        {
            if (property.Name == attr.Name)
            {
                ...
                {
                    property.SetValue(obj, attr.Value.ParseMacros(_ioCore));
                }

                if (property.PropertyType == typeof(StringName))
                {
                    property.SetValue(obj, new StringName(attr.Value.ParseMacros(_ioCore)));
                }
                else if (property.PropertyType == typeof(int))
                {
                    property.SetValue(obj, attr.Value.ParseMacrosToInt(_ioCore));
                }
                else if (property.PropertyType == typeof(float))
                {
                    property.SetValue(obj, attr.Value.ParseMacrosToFloat(_ioCore));
                }
                else if (property.PropertyType == typeof(Vector2))
                {
                    property.SetValue(obj, ToVector2(attr.Value, _ioCore));
                }
                else if (property.PropertyType == typeof(Vector3))
                {
                    property.SetValue(obj, ToVector3(attr.Value, _ioCore));
                }
                else if (property.PropertyType == typeof(Vector4))
                {
                    property.SetValue(obj, ToVector4(attr.Value, _ioCore));
                }
                else if (property.PropertyType == typeof(Vector2I))
                {
                    property.SetValue(obj, ToVector2I(attr.Value, _ioCore));
                }
                else if (property.PropertyType == typeof(Vector3I))
                {
                    property.SetValue(obj, ToVector3I(attr.Value, _ioCore));
                }
                ...
                {
                    property.SetValue(obj, attr.Value.ParseMacrosToLong(_ioCore));
                }
                else if (property.PropertyType == typeof(double))
                {
                    property.SetValue(obj, attr.Value.ParseMacrosToDouble(_ioCore));
                }
                ...
                {
                    property.SetValue(obj, LoadImageTexture(_ioCore, attr.Value.ParseMacros()));
                }
        ...
        foreach (var c in node.ChildNodes)
        {
            ...
            if (childNode.Name == "MenuItem")
            {
                if (childNode.Attributes != null && childNode.Attributes["Label"] != null)
                {
                    ...
                    {
                        var icon = LoadImageTexture(_ioCore, childNode.Attributes["Icon"].Value);
    ...
    public void ConstructChildren(XmlNode node, Node n)
    {
        ...
        {
            var childNode = (XmlNode)c;
            var child = Construct(childNode, _ioCore);
            if (child != null)
            {
                n.AddChild(child);
            }
    ...
    private Vector2 ToVector2(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
    ...
    private Vector3 ToVector3(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
    ...
    private Vector4 ToVector4(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
    ...
    private Vector2I ToVector2I(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
    ...
    private Vector3I ToVector3I(string str, IoCore assetIo = null)
    {
        var parts = str.Split(',');
